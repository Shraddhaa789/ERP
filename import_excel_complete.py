...
def import_inward_data(df):
    """Import inward data with all 57 Excel columns mapped to database columns"""
    conn = get_conn()
    cur = conn.cursor()
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            # Map Excel columns to database fields (excluding id and extra fields)
            values = (
                ...
                safe_str(row.get('Total')),  # additional_total (the 29th column)
                None,  # task_id (extra field)
                None,  # ch_invoice_no (extra field)
                None,  # ch_invoice_date (extra field)
                None,  # ch_submitted (extra field)
                None   # ch_remark (extra field)
            )
            
            # Verify we have exactly 62 values (excluding id)
            if len(values) != 62:
                print(f"Error: Expected 62 values, got {len(values)} for record {imported_count + 1}")
                continue
            
            # Insert into database with all columns (excluding auto-increment id)
            cur.execute("""
...
def import_outward_data(df):
    """Import outward data with all columns"""
    conn = get_conn()
    cur = conn.cursor()
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            values = (
                ...
                None,  # out_from_location (extra field)
                None,  # out_to_location (extra field)
                None,  # out_tracking (extra field)
                None   # out_distance_km (extra field)
            )
            
            # Verify we have exactly 42 values (excluding id)
            if len(values) != 42:
