...
def inward_db_values(record):
    """Create database values tuple matching the 62 columns in basic_info table"""
    return (
...
def outward_db_values(record):
    """Create database values tuple for outward_info table"""
    return (
        ...
        db_scalar(record.get("out_from_location")),
        db_scalar(record.get("out_to_location")),
        db_scalar(record.get("out_tracking")),
        db_scalar(record.get("out_distance_km"))
    )

def import_inward_data(df):
    """Import inward data with all 57 Excel columns mapped to 62 database columns"""
    conn = get_conn()
    cur = conn.cursor()
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            record = {
                ...
                "additional_total": safe_str(row.get('Total')),  # The 29th column from Excel
                "task_id": None,  # Extra field - set to None
                "ch_invoice_no": None,  # Extra field - set to None
                "ch_invoice_date": None,  # Extra field - set to None
                "ch_submitted": None,  # Extra field - set to None
                "ch_remark": None  # Extra field - set to None
            }
            
            # Use the database values function
            values = inward_db_values(record)
            
            # Verify we have exactly 62 values
            if len(values) != 62:
                print(f"Error: Expected 62 values, got {len(values)} for record {imported_count + 1}")
                continue
            
            # Insert into database using all 62 columns
            cur.execute("""
                INSERT INTO basic_info (
                    ...
                    additional_total,
                    task_id, ch_invoice_no, ch_invoice_date, ch_submitted, ch_remark
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            record = {
                ...
                "out_from_location": None,  # Extra field
                "out_to_location": None,    # Extra field
                "out_tracking": None,       # Extra field
                "out_distance_km": None     # Extra field
            }
            
            # Use the database values function
            values = outward_db_values(record)
            
            # Verify we have exactly 42 values
            if len(values) != 42:
                print(f"Error: Expected 42 values, got {len(values)} for record {imported_count + 1}")
                continue
            ...
            cur.execute("""
                INSERT INTO outward_info (
                    ...
                    out_from_location, out_to_location, out_tracking, out_distance_km
