...
def import_inward_data(df):
    """Import inward data with all 57 columns"""
    conn = get_conn()
    cur = conn.cursor()
    ...
    try:
        ...
        for index, row in df.iterrows():
            ...
            # Map Excel columns to database fields using all 57 columns
            values = (
                ...
                safe_str(row.get('Total')),  # additional_total (this was missing before!)
            ...
            # Verify we have exactly 57 values
            if len(values) != 57:
                print(f"Error: Expected 57 values, got {len(values)} for record {imported_count + 1}")
                continue
            
            # Insert into database with all 57 columns
            cur.execute("""
